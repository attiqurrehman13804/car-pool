...
app.get("/", (req, res) => {
  res.json({ message: "Welcome to the Car Pool API", version: "1.0.0" });
});
...
server.listen(config.port, () => {
  console.log(`Car Pool API running on http://192.168.100.71:${config.port}`);
  console.log(
    `Allowed email domains: ${config.allowedEmailDomains.join(", ")}`,
  );
});
